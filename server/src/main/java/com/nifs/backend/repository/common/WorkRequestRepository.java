...
public interface WorkRequestRepository extends JpaRepository<WorkRequest,String> {
    ...
    List<WorkRequest> findAllByOrderByCreatedOnDesc();

    List<WorkRequest> findByDivisionIdOrderByCreatedOnDesc(String division);

    @Transactional
    @Modifying
    @Query("UPDATE WorkRequest r SET r.hodApproved = :hodApproved, r.modifiedBy = :modifiedBy, r.modifiedOn = :modifiedOn WHERE r.documentNo = :documentNo")
    ...
    @Transactional
    @Modifying
    @Query("UPDATE WorkRequest r SET r.dirApproved = :dirApproved, r.modifiedBy = :modifiedBy, r.modifiedOn = :modifiedOn WHERE r.documentNo = :documentNo")

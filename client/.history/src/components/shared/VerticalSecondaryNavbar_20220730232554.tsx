                    ...
                    <>
                        ...
                        {content.routes.map((page: any, i: number) => (
                            isAdmin ? <Link to={page.link} key={i}>
                                <button className={location.pathname === page.link ? 'px-4 py-2 rounded-2xl text-black hover:text-yellow-300' : 'text-white px-4 py-2 hover:text-yellow-300'}><span ><img src={page.img} alt={page.title} className='w-10' /></span>{page.title}</button>

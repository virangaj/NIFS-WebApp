                    ...
                    <>
                        ...
                        {content.routes.map((page: any, i: number) => (
                            isAdmin ? <Link to={page.link} key={i}>
                                <button className={location.pathname === page.link ? 'my-[1vh] px-4 py-2 rounded-2xl text-black hover:text-yellow-300' : 'text-white px-4 py-2 hover:text-yellow-300'}>{page.title}</button>
                            </Link> : (!page.only_admin ? <Link to={page.link} key={i}>
                                <button className={location.pathname === page.link ? 'my-[1vh] px-4 py-2 rounded-2xl text-black' : 'text-white px-4 py-2'}>{page.title}</button>
                            </Link> : "")
                        ))}
                    </>

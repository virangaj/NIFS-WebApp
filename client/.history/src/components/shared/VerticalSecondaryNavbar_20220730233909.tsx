                        ...
                        {content.routes.map((page: any, i: number) => (
                            isAdmin ? <Link to={page.link} key={i}>
                                <div className='flex items-center'>
                                    ...
                                    <button className={location.pathname === page.link ? 'px-4 py-2 rounded-2xl text-black font-semibold hover:text-blue-500 hover:font-semibold' : 'text-gray-500 px-4 py-2 hover:text-blue-500 hover:font-semibold'}>{page.title}</button>
                                </div>

...
const HODTransportCost = () => {
  ...
  const retriveData = () => {
    setLoading(true);
    setTimeout(() => {
      TransportCostService.getDivisionTransportCost(
        auth?.user?.token,
        auth?.division
      )
        .then((res) => {
  ...
  const sendReject = () => {
    ...
    setLoading(true);
    setTimeout(() => {
      TransportCostService.sendHodApproval(
        selectedData,
        auth?.user?.token,
        ...
        .then((res) => {
          if (res.data) {
            toast.warning("Transport Cost Request is Declined");
          } else {
...
export default HODTransportCost;

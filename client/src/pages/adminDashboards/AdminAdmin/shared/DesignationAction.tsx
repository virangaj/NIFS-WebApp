...
function DesignationAction({ params, rowId, setRowId, setDeleteId }:any) {
    ...
    const handleUpdate = async () => {
		setLoading(true);
		const { id, designationName, location } = params.row;
		setTimeout(async () => {
    ...
    const handleDelete = async () => {
		...
		setTimeout(async () => {
			const { id } = params.row;
			const result = await DesignationMasterService.deleteDesignation(id);
                    ...
                    <div className='flex flex-col ml-3'>
                        <div className='mb-2 font-medium leading-none'>
                            ...
                            <code className='px-2 bg-red-200 rounded-lg'>
                                {' '}
                                {params.row.id} - {params.row.designationName}
                            </code>
                            ?
                        </div>
                        <p className='mt-1 text-sm leading-none text-gray-600'>
                            By deleting this Employee Designation you will lose this employee
                            type data
                        </p>
                    </div>
                </div>

...
const ProcumentSRN = () => {
  ...
  const retriveData = () => {
    setLoading(true);
    setTimeout(() => {
      SRNService.getDivisionSRN(auth?.user?.token, auth?.division)
        .then((res) => {
  ...
  const sendApprove = () => {
    ...
    setTimeout(() => {
      SRNService.sendHodApproval(
        ...
        .then((res) => {
          if (res.data) {
            toast.success("SRN Request is Confirmed");
          } else {
  ...
  const sendReject = () => {
    ...
    setTimeout(() => {
      SRNService.sendHodApproval(
        ...
        .then((res) => {
          if (res.data) {
            toast.warning("SRN Request is Declined");
          } else {
            toast.error("Request cannot be performed");
          }
        })
